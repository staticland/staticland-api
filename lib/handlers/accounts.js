...
module.exports = function (db, config) {
  ...
  function register (req, res, ctx) {
    ...
    /* else if admin user, let them make an account for people */
  } else if (scopes.app && scopes.app.admin) {
      township.verify(req, function (err, account, token) {
        ...
        township.accounts.authorize(account.auth.key, [scopes.app.admin], function (err) {
          if (err) return error(403, 'Server requires API access scope').pipe(res)
          createAccount()
        })
      })
    } else {
      return error(403, 'Registration not available. Apply to use the hosted service: https://static.land#apply').pipe(res)
    }
  }
