...
module.exports = function staticland (options) {
  ...
  app.on('/sites', function (req, res, ctx) {
    if (req.method === 'POST') {
      ...
      function firstDeploy () {
        data.access.scopes.push(deployScope)

        app.access.update(data.basic.key, data.access.scopes, function (err, accessData) {
      ...
      function deploy () {
        app.access.verify(data.basic.key, [deployScope], function (err) {
          if (err) return app.error(res, '403', 'Authorization failed')
  ...
  app.on('/auth', function (req, res, ctx) {
    ...
    if (req.method === 'POST') {
      ...
      app.error(res, 405, 'Method now Implemented')
    } else {
      app.error(res, 405, 'Method not allowed')
    }
  })
