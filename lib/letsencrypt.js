  ...
  le.renew = function renew (options, callback) {
    ...
    letsencrypt.getCert({
      ...
      challenge: function challenge (domain, challengepath, data, done) {
        var siteDir = path.join(config.dirs.sitesDir, domain)
        var challengeDir = path.join(siteDir, 'html', '.well-known', 'acme-challenge')
        var filename = challengepath.split('/acme-challenge/')[1]
      ...
      console.log('weaaaaaaaaaaaaoaaaoaooaooaoaaoaoaoaoaooaaooaoaoaoaaoaoaooaaooaoaaooa')
      console.log(err, cert, key, ca, account)
      console.log('iieieieieieiieieieieieieieieiieiieieiieieieieieieieiieieieieieieeiei')
      if (err) return callback(err)
